...
# def jaxcalcDistFromBall(x0, x, fs):
#     #Y is a list of points x1...xm
#     if len(fs) != 0:
...
points = np.reshape(np.fromfile('Heu2.txt', dtype=int, count=-1, sep=' '), (-1, 6))/200
...
points_new = [points[j] for j in range(len(points))]
...
#%%
delta = 0.001
domainSets = [odl.set.domain.IntervalProd(x-delta,x+delta) for x in points_new]
...
pointToFind = np.array([136,189,25, 0,0,0])/200

#%%
def grad2(x):
    return 10e4*(func2(x) - func2(x+10e-4*(func2(x)-pointToFind)))
...
#%%
def funcCutOffSet(x):
    y = (func2(x)-pointToFind)
    return y[0]**2+y[1]**2+y[2]**2

# def gradCutOffSet(x):
#     y = 2*grad(x)@(func2(x)-pointToFind)
#     y[3] = 0
#     y[4] = 0
#     y[5] = 0
#     return y
...
class myOperator(odl.Operator):
    ...
    def __init__(self, dome, rang):
        ...
        dom = dome
        ran = rang
        super(myOperator, self).__init__(dom, ran, linear=False)
    # def _call(self, x, out):
    #     out = funcCutOffSet(x)
    def _call(self, x):
        return funcCutOffSet(x)
    # def _call(self, x, out):
    #     out = funcCutOffSet(x)
    @property
    def gradient(self):
        ...
        class MyGradientOperator(odl.Operator):
            ...
            def __init__(self):
                ...
                super(MyGradientOperator, self).__init__(
                    domain=functional.domain, range=odl.rn(6, dtype='float32'))
...
xStart = domain.element()
print(xStart)

def thing(f, x, line_search=1.0, maxiter=1000, tol=1e-16,
    ...
    y = x
    grad_x = grad(y)#grad.range.element()
    for _ in range(maxiter):
        grad_x = grad(y)#grad(x, out=grad_x)

        dir_derivative = -np.linalg.norm(grad_x) ** 2
        if np.abs(dir_derivative) < tol:
            return  # we have converged
        step = 0.1#line_search(x, -grad_x, dir_derivative)
...
#line_search = odl.solvers.BacktrackingLineSearch(op, max_num_iter=10)
y = thing(op, xStart, maxiter=200)#, line_search=line_search)
